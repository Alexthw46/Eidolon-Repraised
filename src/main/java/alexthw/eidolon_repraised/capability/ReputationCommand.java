...
public class ReputationCommand {
    ...
    private static int apply(CommandSourceStack sources, Collection<? extends Player> players, BiConsumer<Player, CommandSourceStack> action) {
        ...
        if (players.size() == 1) {
            sources.sendSuccess(() -> Component.translatable("commands.eidolon_repraised.reputation.success.single", players.iterator().next().getDisplayName()), true);
        } else {
            sources.sendSuccess(() -> Component.translatable("commands.eidolon_repraised.reputation.success.multiple", players.size()), true);
        }
        return players.size();
    }
    ...
    public static class DeityArgument implements ArgumentType<Deity> {
        private static final DynamicCommandExceptionType UNKNOWN = new DynamicCommandExceptionType((obj) -> Component.translatable("argument.eidolon_repraised.deity.unknown", obj));
