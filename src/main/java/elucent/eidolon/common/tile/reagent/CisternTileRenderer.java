...
public class CisternTileRenderer implements BlockEntityRenderer<CisternTileEntity> {
    ...
    @Override
    public void render(CisternTileEntity tile, float partialTicks, @NotNull PoseStack matrixStackIn, @NotNull MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        Minecraft mc = Minecraft.getInstance();
        if (tile.getBlockState().getValue(PillarBlockBase.BOTTOM) && tile.downPressure < 1 && tile.tank.getPressure() < 1 / 64f)
            return;
